...
d2 = np.array(d2)
import math
CX, CY = 3066.215, 2090.868
order = np.argsort(d2)[::-1]
print("worst model nodes (x, y, z | r, angle deg, dist):")
bins = {}
for i in order[:40]:
    x, y, z = sel[i]; r = math.hypot(x - CX, y - CY); a = math.degrees(math.atan2(y - CY, x - CX))
    print("  (%.3f, %.3f, %.3f) | r %.3f ang %.1f dist %.4f" % (x, y, z, r, a, d2[i]))
over = sel[d2 > 0.05]
print("nodes with dist > 0.05: %d; z range %.2f..%.2f; r range %.2f..%.2f" % (len(over), over[:, 2].min() if len(over) else 0, over[:, 2].max() if len(over) else 0,
      min(math.hypot(p[0]-CX, p[1]-CY) for p in over) if len(over) else 0, max(math.hypot(p[0]-CX, p[1]-CY) for p in over) if len(over) else 0))
angs = sorted(round(math.degrees(math.atan2(p[1]-CY, p[0]-CX))) for p in over)
print("angles of far nodes:", angs[:80])
