...
def main():
    ...
    for k in range(len(runs) - 1):
        ...
        bounds[k] = 0.5 * (a + b)
    zb = [lo[ax]] + [bounds[k] for k in range(len(runs) - 1)] + [hi[ax]]
    # group thin consecutive levels with the same loop count and monotonic area into taper runs
    groups = []   # (first level, last level, kind)
    k = 0
    while k < len(runs):
        j = k
        thin = lambda q: (zb[q + 1] - zb[q]) < 0.08 * (hi[ax] - lo[ax])
        if thin(k):
            n0 = sig[(runs[k][0] + runs[k][1]) // 2][0]
            areas = [sig[(runs[k][0] + runs[k][1]) // 2][1]]
            while j + 1 < len(runs) and thin(j + 1) and sig[(runs[j + 1][0] + runs[j + 1][1]) // 2][0] == n0:
                a_next = sig[(runs[j + 1][0] + runs[j + 1][1]) // 2][1]
                mono = len(areas) < 2 or (a_next - areas[-1]) * (areas[-1] - areas[-2]) > 0
                if not mono: break
                areas.append(a_next); j += 1
        groups.append((k, j, "taper" if j - k >= 2 else "prism"))
        k = j + 1
    print("groups: %s" % [(g[0], g[1], g[2]) for g in groups], flush=True)
    parts = []
    def section_wires(h):
        Ls = [dedupe(l) for l in loops(section2d(tri, ax, h)) if len(l) > 3]
        Ls = [l for l in Ls if abs(loop_area(np.r_[l, l[:1]])) > 1e-3]
    ...
    for g0, g1, kind in groups:
        z_lo, z_hi = zb[g0], zb[g1 + 1]
        if kind == "taper":
            eps = 0.02 * (z_hi - z_lo)
            A, B = section_wires(z_lo + eps), section_wires(z_hi - eps)
            if len(A) == len(B) and len(A) >= 1:
                built = []
                okall = True
                for LA, LB in zip(A, B):
                    # each loop pair -> ruled loft between the two planar outlines, placed at the exact boundaries
                    ts = BRepOffsetAPI_ThruSections(True, True)
                    ts.AddWire(build_wire(*segment_loop(LA), ax, z_lo)); ts.AddWire(build_wire(*segment_loop(LB), ax, z_hi))
                    ts.CheckCompatibility(True)
                    try:
                        ts.Build(); built.append(ts.Shape())
                    except Exception:
                        okall = False; break
                if okall:
                    # outer loft minus hole lofts
                    solid = built[0]
                    from OCP.BRepAlgoAPI import BRepAlgoAPI_Cut
                    for hsld in built[1:]:
                        solid = BRepAlgoAPI_Cut(solid, hsld).Shape()
                    parts.append(solid)
                    print("  taper levels %d-%d: z %.3f..%.3f loops %d (loft)" % (g0, g1, z_lo, z_hi, len(A)), flush=True)
                    continue
            print("  taper levels %d-%d fell back to prisms (loop mismatch)" % (g0, g1), flush=True)
            for q in range(g0, g1 + 1):
                i0, i1 = runs[q]
                solids, nl = build_level(tri, ax, zb[q], zb[q + 1], hs[(i0 + i1) // 2]); parts.extend(solids or [])
            continue
        for q in range(g0, g1 + 1):
            i0, i1 = runs[q]
            solids, nl = build_level(tri, ax, zb[q], zb[q + 1], hs[(i0 + i1) // 2])
            print("  level %d: z %.3f..%.3f loops %d solids %d" % (q, zb[q], zb[q + 1], nl, len(solids or [])), flush=True)
