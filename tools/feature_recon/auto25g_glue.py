...
def main():
    ...
    import os
    glue = os.environ.get("GLUE", "")
    shape = parts[0]
    for s in parts[1:]:
        if glue:
            from OCP.BOPAlgo import BOPAlgo_GlueShift, BOPAlgo_GlueFull
            from OCP.TopTools import TopTools_ListOfShape
            fu = BRepAlgoAPI_Fuse(); a1 = TopTools_ListOfShape(); a1.Append(shape); a2 = TopTools_ListOfShape(); a2.Append(s)
            fu.SetArguments(a1); fu.SetTools(a2)
            fu.SetGlue(BOPAlgo_GlueShift if glue == "shift" else BOPAlgo_GlueFull)
            fu.Build(); shape = fu.Shape()
        else:
