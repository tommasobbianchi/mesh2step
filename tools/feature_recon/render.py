...
for name, (elev, azim) in views.items():
    ...
    ax.set_xlabel("X"); ax.set_ylabel("Y"); ax.set_zlabel("Z"); ax.set_title("mechparts/9 - %s" % name)
    fig.savefig("%s/p9_%s.png" % (sys.argv[2], name), dpi=80, bbox_inches="tight"); plt.close(fig)
