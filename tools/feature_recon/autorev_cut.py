...
# 3b. hole clusters from the remaining candidates: normals perpendicular to a common direction
rest = idx[~used[idx]]
holes = []
if len(rest) > 20:
    N = nrm[rest]; w, v = np.linalg.eigh(N.T @ N); hdir = v[:, 0]   # direction least represented in the normals
    perp = np.abs(N @ hdir) < 0.1
    pts = cent[rest][perp]
    if len(pts) > 20:
        u = np.cross(hdir, [1, 0, 0] if abs(hdir[0]) < 0.9 else [0, 1, 0]); u /= np.linalg.norm(u); vv = np.cross(hdir, u)
        P2 = np.c_[pts @ u, pts @ vv]; c2, rr, dev = fit_circle(P2)
        if dev < max(5 * TOL, 0.05 * rr):
            base = c2[0] * u + c2[1] * vv
            tmin, tmax = (pts @ hdir).min(), (pts @ hdir).max()
            holes.append((hdir, base + tmin * hdir, rr, tmax - tmin))
