...
idx = np.where(cand)[0]
# 3a. planes from the feature map (area share, not triangle count: a big flat face can be 2 triangles)
from features import patches
on_env0 = (np.abs(rc - env_r) <= TOL * 2) | ((radial_n > 0.995) & (np.abs(rc - env_r) <= TOL * 20)) | (axial_n > 0.995)
PATCHES = patches(tri, ~on_env0, TOL, min_share=0.01)
planes = [p for p in PATCHES if p["kind"] == "plane"]
used = np.zeros(len(tri), bool)
flats = []; slots = []
paired = set()
for i, p in enumerate(planes):
    for j in range(i + 1, len(planes)):
        q = planes[j]
        # slot walls: antiparallel normals pointing TOWARD each other (void between), offsets giving a positive width
        if p["normal"] @ q["normal"] < -0.999:
            gap = -(q["offset"] + p["offset"])     # p.x = op, q.x = oq with nq = -np  ->  width along np = -oq - op
            if gap > TOL:
                slots.append((p, q, gap)); paired.update((i, j))
for i, p in enumerate(planes):
    if i not in paired:
        flats.append((p["normal"], p["offset"], p["tris"]))
print("slots %d: %s" % (len(slots), [(np.round(p["normal"], 2).tolist(), round(g, 3)) for p, q, g in slots]), flush=True)
...
for p, q, gap in slots:
    n = np.array(p["normal"]); P = np.vstack([p["points"], q["points"]])
    zdir = np.zeros(3); zdir[ax] = 1.0
    # frame: X = wall normal, Z = spin axis if perpendicular to the normal, else a perpendicular of n
    zz = zdir - (zdir @ n) * n
    if np.linalg.norm(zz) < 1e-6:
        zz = np.cross(n, [1, 0, 0] if abs(n[0]) < 0.9 else [0, 1, 0])
    zz /= np.linalg.norm(zz); yy = np.cross(zz, n)
    lx = P @ n; ly = P @ yy; lz = P @ zz
    x0 = p["offset"]; x1 = -q["offset"]
    frame = gp_Ax2(gp_Pnt(0, 0, 0), gp_Dir(*zz), gp_Dir(*n))
    t = gp_Trsf(); t.SetTransformation(gp_Ax3(frame))
    from OCP.BRepPrimAPI import BRepPrimAPI_MakeBox as MB2
    ext = 2 * Rmax
    ylo, yhi = ly.min() - ext, ly.max() + ext
    # slot floor: an unpaired plane whose normal is along yy, lying between the walls; material is kept on the
    # side opposite its normal, so the cut only goes from the floor outward (along the floor normal)
    for fl in planes:
        m = np.array(fl["normal"])
        if abs(abs(m @ yy) - 1) > 1e-3: continue
        fx = fl["points"] @ n
        if fx.min() < min(x0, x1) - TOL or fx.max() > max(x0, x1) + TOL: continue
        floor_y = float(np.median(fl["points"] @ yy))
        if m @ yy > 0: ylo = floor_y
        else: yhi = floor_y
        print("slot floor at %.3f (normal %s)" % (floor_y, np.round(m, 2).tolist()), flush=True)
        break
    bx = MB2(gp_Pnt(min(x0, x1), ylo, lz.min() - TOL), gp_Pnt(max(x0, x1), yhi, lz.max() + TOL)).Shape()
    bx = BRepBuilderAPI_Transform(bx, t.Inverted(), True).Shape()
    shape = BRepAlgoAPI_Cut(shape, bx).Shape()
