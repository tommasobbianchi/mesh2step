...
for ax in axes:
    polys = []
    for h in np.linspace(lo[ax], hi[ax], 42)[1:-1]:
        for l in loops(section2d(tri, ax, h)):
            l = dedupe(l)
            if len(l) > 3 and abs(loop_area(np.r_[l, l[:1]])) > 1e-3:
                try:
                    pg = Polygon(l)
    ...
    if sil.geom_type == "MultiPolygon": sil = max(sil.geoms, key=lambda g: g.area)
    sil = sil.simplify(TOL * 0.5, preserve_topology=True)
    outer = np.array(sil.exterior.coords)[:-1]
    try:
    ...
    print("axis %s silhouette base volume %.1f (mesh %.1f, excess %+.1f%%)" % ("XYZ"[ax], bv, mvol, 100 * (bv - mvol) / mvol), flush=True)
...
# 3b. slots: each plane pairs one-to-one with its nearest facing plane
paired = set()
pairs = []
for i, p in enumerate(planes):
    best = None
    for j, q in enumerate(planes):
        if j == i or p["normal"] @ q["normal"] >= -0.999: continue
        gap = -(q["offset"] + p["offset"])
        if gap <= TOL: continue
        # the two walls must overlap in their extent perpendicular to the normal
        n = np.array(p["normal"]); qa = q["points"] - np.outer(q["points"] @ n, n); pa = p["points"] - np.outer(p["points"] @ n, n)
        if np.linalg.norm(qa.mean(0) - pa.mean(0)) > 0.5 * (np.ptp(pa, axis=0).max() + np.ptp(qa, axis=0).max()): continue
        if best is None or gap < best[0]: best = (gap, j)
    if best is not None:
        pairs.append((i, best[1]))
mutual = [(i, j) for i, j in pairs if (j, i) in pairs and i < j]
for i, j in mutual:
