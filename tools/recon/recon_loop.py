...
def main():
    ...
    Work like an engineer REVERSING THE DESIGN HISTORY, never modelling the final faces directly: find the sharp
    base shape first (project the maximum extents onto the sketch plane and extrude them; a full round or half
    pipe is a sharp edge that was filleted afterwards), then remove-in-reverse the modifiers and re-apply them on
    top of the base: holes, pockets, chamfers, fillets/rounds. Every program should read sketch -> extrude ->
    modify.
    """).strip()
    if JUNC_SECTION:
        BRIEF = BRIEF + "\n\n" + JUNC_SECTION
    owner = os.environ.get("RECON_OWNER_NOTES")          # voice-interview preview (tools/interview): design intent
    if owner and os.path.exists(owner):
        BRIEF += ("\n\nThe part's OWNER described it in an interview. Treat it as design intent: name features the way"
                  " they do, and where their measurements disagree with the mesh facts, THEIR measurements win.\n"
                  + Path(owner).read_text())

    SEED_NOTE = (" It was generated by reversing the design history (silhouette sketch -> extrude -> rounds): keep "
                 "that sketch -> extrude -> modify structure and correct or add the features the measurement names.")
    history = []
    ...
    def converged(rep):
        ...
        fz = rep["features"]
        return (rep["valid"] and max(rep["p95_mesh_to_solid"], rep["p95_solid_to_mesh"]) <= stop
                and fz["curved"] == fz["patches"] and rep.get("nonanalytic", 0) == 0)

    seed = os.environ.get("RECON_SEED_PROGRAM")           # round 0: reverse.py's design history, no model call
    if seed and os.path.isfile(seed) and not history:
        import shutil
        py0, step0 = WD / "recon_0.py", WD / "recon_0.step"
        shutil.copy(seed, py0)
        ok0, err0 = run_script(py0, step0)
        rep0 = None
        if ok0:
            try:
                import canon
                canon.canonicalize_step(step0)
                rep0 = gate(step0)
            except Exception as e:  # noqa: BLE001
                print(f"seed not measurable: {e}", flush=True)
        if rep0 and rep0["valid"] and abs(rep0["volume_delta_pct"]) <= 5.0:   # a far-off seed would mislead
            history.append({"py": str(py0), "report": rep0, "error": "", "seed": True})
            json.dump(history, open(WD / "history.json", "w"), indent=1)
            write_best(history, diag, WD)
            print("seed (reverse.py design history): " + json.dumps(
                {k: v for k, v in rep0.items() if k not in ("where_wrong", "feature_misses", "nonanalytic_faces")}), flush=True)
            if converged(rep0):
                print("converged at the seed: no model call"); ITERS = 0
        else:
            print(f"seed discarded: {'did not run: ' + err0[-200:] if not ok0 else 'volume/validity off'}", flush=True)
    start = len(history) + 1
    for it in range(start, start + ITERS):
        ...
        if not history:
            ...
            Your previous program is {prev['py']} (read it).{SEED_NOTE if prev.get('seed') else ''} It was executed and measured against the mesh:
